...
def spam(phone: str, amount: int):
    print(f"🚀 Bắt đầu spam {phone} - {amount} lần")
...
async def spam_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if not phone.startswith("0") or len(phone) != 10:
            await update.message.reply_text("❌ Số điện thoại không hợp lệ (phải 10 số, bắt đầu bằng 0)")
            return
